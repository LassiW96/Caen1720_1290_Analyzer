...
void MainWindow::setupDecodeTab() {
  ...
  QHBoxLayout *decodeBtnLayout = new QHBoxLayout();

  m_decodeBtn = new QPushButton("Decode ADC (V1720)");
  m_decodeBtn->setMinimumHeight(40);
  connect(m_decodeBtn, &QPushButton::clicked, this, &MainWindow::startDecode);
  decodeBtnLayout->addWidget(m_decodeBtn);

  m_tdcDecodeBtn = new QPushButton("Decode TDC (V1290)");
  m_tdcDecodeBtn->setMinimumHeight(40);
...
void MainWindow::setupAnalysisTab() {
  ...
  connect(browseAnalysisBtn, &QPushButton::clicked, this,
          &MainWindow::browseAnalysisFile);
  ...
  m_scriptEditor->setPlaceholderText(
      "// Enter ROOT C++ script here...\n// e.g.\n// TH1F *h = "
      "(TH1F*)gDirectory->Get(\"h1\");\n// h->Draw();");
  ...
  connect(runScriptBtn, &QPushButton::clicked, this,
          &MainWindow::runAnalysisScript);
  connect(loadScriptBtn, &QPushButton::clicked, this,
          &MainWindow::loadAnalysisScript);
  connect(loadExampleBtn, &QPushButton::clicked, this,
          &MainWindow::loadExampleScript);
  ...
  connect(m_histList, &QListWidget::itemClicked, this,
          &MainWindow::onHistogramSelected);
  ...
  rightLayout->addWidget(m_previewLabel,
                         4); // large stretch — dominates the space
...
void MainWindow::browseInputFile() {
  QString fileName = QFileDialog::getOpenFileName(
      this, "Select EVIO File", "", "EVIO Files (*.evio *.evio.*)");
  if (!fileName.isEmpty()) {
...
void MainWindow::startTdcDecode() {
  QString inputFile = m_inputFileEdit->text();
  QString outputDir = m_outputDirEdit->text();
  ...
  std::thread([this, inputFile, outputDir]() {
    try {
      V1290EvioDecode decoder(inputFile.toStdString(), outputDir.toStdString());
      decoder.decode();

      QMetaObject::invokeMethod(
          this,
          [this, outputDir, inputFile]() {
            m_logArea->append("TDC decoding finished successfully.");

            // Print Branch Summary
            std::filesystem::path p(inputFile.toStdString());
            while (p.has_extension())
              p = p.stem();
            std::string outputFileName = p.string() + "_tdc_decoded.root";
            std::filesystem::path fullPath =
                std::filesystem::path(outputDir.toStdString()) / outputFileName;
            if (outputDir.isEmpty())
              fullPath = outputFileName;
...
void MainWindow::browseAnalysisFile() {
  QString fileName = QFileDialog::getOpenFileName(this, "Select ROOT File", "",
                                                  "ROOT Files (*.root)");
  if (!fileName.isEmpty()) {
...
void MainWindow::onHistogramSelected(QListWidgetItem *item) {
  ...
  if (h) {
    ...
    TCanvas *oldC =
        (TCanvas *)gROOT->GetListOfCanvases()->FindObject("c_preview");
    if (oldC)
      delete oldC;
...
void MainWindow::loadAnalysisScript() {
  QString fileName = QFileDialog::getOpenFileName(this, "Select Script", "",
                                                  "C++ Macros (*.C *.cxx)");
  if (!fileName.isEmpty()) {
    QFile file(fileName);
...
void MainWindow::runAnalysisScript() {
  ...
  // Save to temp file
  // To avoid ROOT warnings, the filename must match the function name if it
  // exists
  ...
  if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    ...
    if (activeCanvas) {
      ...
      QPixmap pix(tempImg);
      if (!pix.isNull()) {
        m_previewLabel->setPixmap(pix.scaled(m_previewLabel->size(),
                                             Qt::KeepAspectRatio,
                                             Qt::SmoothTransformation));
      }
      // Do NOT delete activeCanvas immediately, so it can be opened
      // interactively.
    ...
    m_logArea->append("Error: Failed to create temporary script file: " +
                      tempScript);
  }
}
...
void MainWindow::loadExampleScript() {
  ...
  searchPaths
      << appDir + "/resources/temp_script.C"    // installed next to binary
      << appDir + "/../resources/temp_script.C" // build/ -> source root
      << appDir +
             "/../share/V1720/resources/temp_script.C" // CMake install layout
  ...
  if (scriptPath.isEmpty()) {
    scriptPath =
        QFileDialog::getOpenFileName(this, "Locate temp_script.C", QString(),
